    ...
    <div className="p-8 min-h-screen grid-bg">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'High Risk Active', value: highRiskReports.length, icon: AlertTriangle, color: 'text-red-400', bg: 'border-red-500/20 bg-red-500/5' },
          { label: 'Resolution Rate', value: `${resolutionRate}%`, icon: TrendingUp, color: 'text-green-400', bg: 'border-green-500/20 bg-green-500/5' },
          { label: 'Hotspot Areas', value: hotspots.length, icon: MapPin, color: 'text-yellow-400', bg: 'border-yellow-500/20 bg-yellow-500/5' },
        ].map((card, i) => (
            ...
            {highRiskReports.length === 0 ? (
              ...
              <div className="space-y-4">
                {highRiskReports.slice(0, 5).map(report => (
                  <div key={report.id} className="p-4 rounded-xl bg-slate-900/50 border border-red-500/20">
                    ...
                    <div className="text-xs text-slate-300 leading-relaxed bg-slate-800/50 rounded-lg p-3">
                ...
                <div key={area} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-slate-400">{count} laporan</span>
                    </div>
            ...
            <div className="space-y-3">
              {topCategories.filter(c => c.count > 0).map(cat => (
                <div key={cat.key} className="flex items-center gap-3">
                  <div className="text-lg">{cat.key === 'sampah' ? '🗑️' : cat.key === 'banjir' ? '🌊' : cat.key === 'jalan_rusak' ? '🚧' : cat.key === 'lampu_jalan' ? '💡' : '⚠️'}</div>
                  <div className="flex-1">
                    <div className="flex justify-between text-sm mb-1">
              ...
              {reports.slice(0, 6).map(report => (
                <div key={report.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-900/50 border border-slate-700/50">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border border-cyan-500/20 flex items-center justify-center flex-shrink-0">
                    <span className="text-sm font-bold text-cyan-400">{report.ai_score}</span>
                  </div>
